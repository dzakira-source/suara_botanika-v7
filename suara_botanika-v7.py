...
if menu == "🏠 Beranda & Panduan":
    ...
    with col2:
        ...
        """, unsafe_allow_html=True)
        st.image("daun.JPG", caption="Semua anak berhak belajar sains dengan gembira!", use_container_width=True)
...
# -----------------------------------------------------------------------------
    ...
    with col1:
        ...
        st.image(info["image"], use_container_width=True, caption=f"Foto Nyata: {info['title']}")
...
# -----------------------------------------------------------------------------
    ...
    with tabs[0]:
        ...
        for idx, (k, v) in enumerate(daun_types.items()):
            with col_daun[idx]:
                st.markdown(f"### {k}")
                st.image(v["image"], use_container_width=True, caption=f"Foto Daun: {k}")
                st.info(v["desc"])
    ...
    with tabs[1]:
        ...
        for idx, (k, v) in enumerate(batang_types.items()):
            with col_batang[idx]:
                st.markdown(f"### {k}")
                st.image(v["image"], use_container_width=True, caption=f"Foto Batang: {k}")
                st.success(v["desc"])
    ...
    with tabs[2]:
        ...
        for idx, (k, v) in enumerate(akar_types.items()):
            with col_akar[idx]:
                st.markdown(f"### {k}")
                st.image(v["image"], use_container_width=True, caption=f"Foto Akar: {k}")
                st.warning(v["desc"])
...
# -----------------------------------------------------------------------------
    ...
    tabs_foto = st.tabs(["🎮 Game Masak Fotosintesis", "🎥 Video Animasi 3D & Pelajaran"])
    ...
    with tabs_foto[0]:
        ...
        for idx, (nama, info) in enumerate(bahan_tersedia.items()):
            col_idx = idx % 3
            with cols[col_idx]:
                st.image(info["image"], use_container_width=True, caption=nama)
                st.markdown(f"""
    ...
    with tabs_foto[1]:
        st.subheader("🎥 Video Animasi 3D Pembelajaran Fotosintesis")
        video_path = "Proses Fotosintesis.mp4"
        if os.path.exists(video_path):
            st.video(video_path)
            st.success("Berhasil memutar video 'Proses Fotosintesis.mp4' secara lokal!")
        else:
            st.warning("⚠️ File video 'Proses Fotosintesis.mp4' tidak terdeteksi langsung secara lokal.")
            st.info("💡 Memutar video animasi 3D Fotosintesis dari sumber online terpercaya:")
            st.video("https://www.youtube.com/watch?v=sQK3Yr4Sc_k")
            st.image("daun.JPG", caption="Fotosintesis: Sinar matahari menyinari dedaunan hijau", use_container_width=True)
...
# -----------------------------------------------------------------------------
    ...
    with col1:
        ...
        st.image(veg_data[veg_choice]["image"], use_container_width=True, caption=f"Foto: {veg_choice}")
    ...
    with col2:
        ...
        st.image(gen_data[gen_choice]["image"], use_container_width=True, caption=f"Foto: {gen_choice}")
...
# -----------------------------------------------------------------------------
    ...
    with tabs_lokal[0]:
        ...
        with col2:
            st.image("iwel-iwel.JPG", caption="Kue Tradisional dibungkus Daun Pisang Nyata", use_container_width=True)
            st.markdown("""
            <div class="abk-card">
    ...
    with tabs_lokal[1]:
        ...
        with col2:
            st.image("damarkurung.JPG", caption="Lampion Tradisional Damar Kurung yang bersinar indah", use_container_width=True)
            st.markdown("""
            <div class="abk-card">
...
# -----------------------------------------------------------------------------
    ...
    # Define Quiz Questions with Real Unsplash Images
    questions = [
        {
            "no": 1,
    ...
    if st.session_state.quiz_finished:
        ...
        if score == 100:
            ...
            solo_voice = "Luar biasa! Skor kamu seratus. Kamu memahami seluruh materi tumbuhan dan kearifan lokal secara sempurna!"
        elif score >= 80:
            ...
            solo_voice = f"Hebat! Skor kamu {score}. Kamu dapat menghubungkan bagian tumbuhan dengan kebiasaan di sekitar kita dengan sangat baik!"
        elif score >= 40:
        ...
        with col_img:
            ...
            st.image(q["image"], use_container_width=True, caption=f"Gambar Nyata: {q['type']}")
